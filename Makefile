...
DOCKER_RUN := docker run --rm --volume "$(CURDIR)":/srv/jekyll --workdir /srv/jekyll
...
build: ## Build the static site into _site/ (Docker, no local Ruby needed)
	$(DOCKER_RUN) $(IMAGE) sh -c "bundle install && bundle exec jekyll build"
