...
`make dev` serves on port **4260**, this repo's fixed slot in the port
...
Jekyll binds 4260 inside the container as well as on the host, so the
...
`make dev` still refuses to start if another container already holds 4260;
stop that one first.
